...
# ---- SARSA ----
agent.ALPHA = 0.55
agent.MAX_EPISODES = 500

# ---- environment ----
env = agent.MultiGoalCliffWalkingEnv(render_mode="rgb_array")
Q_val, rewards, safe_sarsa, risky_sarsa = agent.SARSA(env)
...
# plot & gif
agent.plot(rewards)
plt.savefig(os.path.join(plots_dir, "sarsa_plot.png"))
plt.close()

agent.create_gif(
    agent.run_and_collect_frames(env, agent.find_policy_epsilon(Q_val, 0)),
    filename=os.path.join(gifs_dir, "sarsa.gif")
)
