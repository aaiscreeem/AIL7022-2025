...
def tdis(env, num_episodes = 10000, seed = 509, gamma = 0.85, alpha = 0.02, epsilon = 0.4):
    """
    ...
    for episode in range(num_episodes):
        ...
        while ((not done) and (t < T)):
            ...
            epsilon_prime = epsilon * (1-(episode/num_episodes))
            pi = find_policy_epsilon(Q, epsilon_prime)
            rho = pi[next_state, next_action]/max(behav_policy[next_state, next_action], 1e-7)
            G = reward
            if not done:
                G += gamma * np.max(Q[next_state]) # Q learning inspired TD instead of SARSA inspired
            Q[state][action] += alpha * (G - Q[state][action]) * rho
            state = next_state
            if t+1 >= T:
                break
            action = next_action
    ...
    def get_target_prob(state, action):
        ...
        """
        return 1 if final_policy[state] == action else 0
...
def generate_policy_gif(env, policy, noise, max_steps=500, fps=4):
    ...
    filename = os.path.join(gifs_dir, f"temporal_difference_gif_({noise}).gif")
...
if __name__ == '__main__':
    ...
    for noise in [0, 0.1, 0.01]:
        ...
        for seed in range(num_seeds):
            ...
            # Train the policy
            q_values, policy, episode_rewards = tdis(env, num_episodes, seed)
        ...
        plot_fname = os.path.join(plots_dir, f"temporal_difference_reward_curve_({NOISE}).png")
        try:
